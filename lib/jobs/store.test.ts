...
import { isStale, STALE_AFTER_MS, type Job } from "./types";
...
describe("job store", () => {
  ...
  test("a render that stopped reporting is surfaced as failed, not stuck", async () => {
    ...
    await store.save(job({ status: "running", progress: 30, updatedAt: stalled }));
...
describe("isStale", () => {
  test("only applies to work that claims to be in flight", () => {
    ...
    expect(isStale(job({ status: "running", updatedAt: old }))).toBe(true);
    expect(isStale(job({ status: "queued", updatedAt: old }))).toBe(true);
    expect(isStale(job({ status: "done", updatedAt: old }))).toBe(false);
    expect(isStale(job({ status: "failed", updatedAt: old }))).toBe(false);
  });
});
